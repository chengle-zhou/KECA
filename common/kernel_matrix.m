...
if strcmp(kernel_type,'RBF_kernel'), % 径向基核函数
    if nargin<4,
...
elseif strcmp(kernel_type,'lin_kernel') % 线性核函数
    if nargin<4,
        omega = Xtrain*Xtrain';
    else
        omega = Xtrain*Xt';
...
elseif strcmp(kernel_type,'poly_kernel') % 多项式核函数
    if nargin<4,
...
elseif strcmp(kernel_type,'wav_kernel') % 小波核函数
    if nargin<4,
        XXh = sum(Xtrain.^2,2)*ones(1,nb_data);
        omega = XXh+XXh'-2*(Xtrain*Xtrain');
...
elseif strcmp(kernel_type,'cos_kernel')
    if nargin<4,
        xx11=(pi/2)*Xtrain*Xtrain'; % 余弦核函数
        omega=(pi/4)*cos(xx11);
    else
        xx22=(pi/2)*Xtrain*Xt';
        omega=(pi/4)*cos(xx22);
